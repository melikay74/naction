...
const MEMBERSHIPS: { name: string; price: string; description: string; tier?: Tier }[] = [
  { name: 'Get Listed', 
    price: 'Free', 
    description: 'Your business name, city and phone number, listed in the network.', },
  {
    name: 'Get Priority',
    price: '$29.99/mo',
          ...
          <div>
            <div className="card tiers-panel">
              ...
              <ul className="tiers-panel-content">
                {/* The free row earns no badge, but still takes a tier class for its ground colour. */}
                {MEMBERSHIPS.map((m) => (
                  <li key={m.name} className={`tier tier-${m.tier ?? 'basic'}`}>
